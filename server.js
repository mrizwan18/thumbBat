...
io.sockets.on('connection', function (socket) {
    ...
    socket.on("createRoom", function (user, fn) {
        ...
        if (opponent !== "Computer") {
            if (sockets[sockets.findIndex(x => x.id == opponent.id)].leave(opponent.id)) {
...
function getOpponent(status) {
    userBattingList.sort(function () { return 0.5 - Math.random(); })
    userBowlingList.sort(function () { return 0.5 - Math.random(); })
    if (status == 2) {
